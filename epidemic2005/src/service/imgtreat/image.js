...
export let imageRotate = (imgObj, rotate) => {
  // 计算旋转后宽高
  let rotateWidth =
    imgObj.width * Math.cos((rotate * Math.PI) / 180) +
    imgObj.height * Math.sin((rotate * Math.PI) / 180);
  let rotateHeight =
    imgObj.height * Math.cos((rotate * Math.PI) / 180) +
    imgObj.width * Math.sin((rotate * Math.PI) / 180);
  // 创建图像
  return create(rotateWidth, rotateHeight).then(canvas => {
    // 取中心点旋转
...
export let imageResize = (imgObj, width, height) => {
  return create(width, height).then(canvas => {
    let context = canvas.getContext("2d");
